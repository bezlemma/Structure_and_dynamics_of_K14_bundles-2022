function [lftloc,rgtloc,lftamp,rgtamp,lftwid,rgtwid,lftamp_err,rgtamp_err,SkipList] = ...
    func_doubleline_fit_forward(data1D,frames,TPH,graph,SkipList,lftloc,rgtloc,...
...
%% ---------------- Fitting parameters
trimStart = 1; %THERE IS AN ERROR HERE BETWEEN HOW idxValid works and how xList works!
trimEnd = xRgt;
ylist = (1/2):(yBotEnd-yBin);
yIntList =  1:(yBotEnd-yBin);
NARROW = 100; %this can be 3.5 at smallest, need as many pixels as we have free variables to fit
...
%     
    ...
    for yy = yIntList
        ...
        if ~isnan(lftloc(fr,yy)) && ~isnan(rgtloc(fr,yy))
            ...
            (lftloc(fr,yy)-NARROW),(rgtloc(fr,yy)-NARROW),... %Min Locations
            (lftloc(fr,yy)+NARROW),(rgtloc(fr,yy)+NARROW),graphgraph); %Max Locations
